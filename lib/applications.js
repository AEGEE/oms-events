...
exports.listUserAppliedEvents = (req, res, next) => {
  Event
    ...
    .elemMatch('applications', { foreign_id: req.user.id })
...
exports.getApplication = (req, res, next) => {
  ...
  const application = event.applications.find(element => element.foreign_id === req.user.id);
...
exports.setApplication = (req, res, next) => {
  ...
  const application = event.applications.find((element, idx) => {
    if (element.foreign_id === req.user.id) {
      index = idx;
      return true;
    }
  ...
  if (application === undefined) {
    event.applications.push({
      ...
      antenna: req.user.primary_body.name,
      antenna_id: req.user.primary_body.id,
