  ...
  Event
    ...
    .exec((err, events) => {
      ...
      const filteredEvents = events.filter((event) => {
        // TODO: Include bodies
        return event.status.visibility.users.includes(req.user.id.toString())
          || intersects(event.status.visibility.roles, []) // TODO implement when core has permissions
      ...
      const retVal = events.filter((event) => {
        ...
        return event.lifecycle.transitions.some((transition) => {
          ...
          return (transition.from.equals(event.status)
            && (transition.allowedFor.users.includes(req.user.id.toString())
              || intersects(transition.allowedFor.roles,[]) // TODO implement when core has permissions
...
exports.listLocalInvolvedEvents = (req, res, next) => {
  ...
  return Event
    .aggregate([
      { $match: { 'applications.antenna_id': String(req.user.primary_body.id) } },
      { $unwind: '$applications' },
      { $match: { 'applications.antenna_id': String(req.user.primary_body.id) } },
      { $group: {
        _id: '$_id',
...
exports.addEvent = (req, res, next) => {
  ...
  user.getDefaultEventRoles((defaultRoles) => {
    ...
    newEvent.organizing_locals = [
      {
        name: req.user.primary_body.name,
        foreign_id: req.user.primary_body.id,
      },
    ];
...
cannot set initial status.`,
        ...
        if (!transition.allowedFor.users.includes(req.user.id.toString())
            && !intersects(transition.allowedFor.roles, []) // TODO implement when core has permissions
            && !intersects(transition.allowedFor.special, req.user.special)) {
...
exports.eventDetails = (req, res, next) => {
  ...
  const transformUserData = (u) => {
    return {
      ...
      antenna_name: u.primary_body.name,
    };
  };
