...
const getUserById = (authToken, id, callback) => {
  ...
  const opts = {
    url: `${config.core.url}:${config.core.port}/api/users/${id}`,
    method: 'GET',
  ...
  httprequest(opts, (requestError, requestResult, body) => {
    ...
    var user = transformUser(body.data);
...
const transformUser = (user) => {
  // TODO rewrite that after CORE-131
  if(user.bodies)
    user.primary_body = user.bodies[0];
  else
    user.primary_body = {id: -1}

  // Special roles
  user.special = ['Public'];

  if (user.is_superadmin) {
    user.special.push('Superadmin');
  }
  if(user.circles.some((circle) => {
    return circle.parent_id == config.global_board_circle;
  })) {
    user.special.push('Board Member');
  }
